...
5b. **Discover skill files** — `find .claude/skills -maxdepth 2 -name SKILL.md` (exclude `init-project/templates/`)
6b. **Parse frontmatter** tiap skill — extract `name:` menggunakan awk:
    ```bash
    awk '/^---/{c++; if(c==2) exit} c==1 && /^name:/{print $2}' <file>
    ```
7b. **Read SKILLS.md** inventory — parse kolom pertama tabel (pattern `| \*\*name\*\*`)
8b. **Compare:**
...
|---|---|---|---|
| git-workflow | ✅ | ✅ | ✅ sync |
| [unregistered-skill] | ✅ | ❌ | ⚠️ unregistered |
