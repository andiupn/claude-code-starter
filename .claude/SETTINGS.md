...
- **`.claude/settings.local.json`** — User-specific configuration (git-ignored)
  - Per-machine, per-OS development tools (git, php, mysql, runtime commands)
  - Local file write permissions (scoped to .claude/ and experiments/)
  - Safety guardrails (deny destructive commands like `rm -rf`, `sudo`)

- **`.claude/settings.local.example.json`** — Template multi-OS (git-tracked, safe to commit)
  - Berisi 4 blok siap pakai: `permissions_WINDOWS`, `permissions_LINUX`, `permissions_MACOS`, `permissions_WSL`
  - Setup baru: copy file ini ke `settings.local.json`, hapus blok OS lain, rename key ke `permissions`

This separation ensures consistency across the team while respecting local development environments.

---

## Setup di Mesin Baru / OS Baru

```bash
# Copy template
cp .claude/settings.local.example.json .claude/settings.local.json

# Edit: hapus blok OS lain, rename key yang dipilih dari
# "permissions_LINUX" → "permissions"
```

Pilihan blok yang tersedia di `settings.local.example.json`:
| Key | Untuk |
|---|---|
| `permissions_WINDOWS` | Windows native (Git Bash / PowerShell) |
| `permissions_LINUX` | Linux native (Ubuntu, Mint, dll) |
| `permissions_MACOS` | macOS (Intel / Apple Silicon) |
| `permissions_WSL` | WSL — Linux di dalam Windows, akses `/mnt/d/` |
...
- **Rationale:** Read-only system diagnostics untuk debugging Windows; safe untuk shared projects
- **Note:** Gunakan `Bash(powershell Get-*)` untuk Windows diagnostics — sudah ada di allowlist
...
- `mcp__computer-use__*` — Computer use tools (click, type, scroll, screenshot)
- **Scope:** `.claude/settings.json`
- **Rationale:** Desktop automation for specific workflows (UI testing, Laragon control)
...
3. **Provide details:**
   - Command/tool pattern: `Bash(npm test)` or `mcp__claude-in-chrome__navigate`
